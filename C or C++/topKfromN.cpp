...
struct Sorting
{
    ...
    {
        return x < y;//sorts in non-decreasing order - toggle for non-increasing order
    }
...
void solve()
{
    // code displays the smallest k numbers from a list of n numbers
    // to display the largest k numbers, use a min heap and toggle the comparison condition in if-block in the for loop
    Heap ob("max");
    cin >> n >> k;
    int h;
    for(int i=0;i<n;i++)
    ...
    ob.buildSorted();
    cout << k << " extreme elements of the heap: ";
    for(int i=0;i<ob.sorted.size();i++)
        cout << ob.sorted[i] << " ";
    cout << endl;
}
int main()
